...
class PlacementCost(object):
    def __init__(
        ...
        self.node_mask = np.array([1] * (self.grid_col * self.grid_row)).reshape(
        ...
        assert (
            len(self.modules_w_pins)
            - self.hard_macro_pins_cnt
            - self.soft_macro_pins_cnt
        ) == self.module_cnt
            ...
            while line:
                ...
                if line_item[0] == "node":
                    ...
                    if node_name == "__metadata__":
                        ...
                        # soft macro pin
                        soft_macro_pin = self.SoftMacroPin(
                            name=node_name,
                            ref_id=None,
                            x=attr_dict["x"][1],
                            y=attr_dict["y"][1],
                        ...
                        if soft_macro_name in self.soft_macros_to_inpins.keys():
                            self.soft_macros_to_inpins[soft_macro_name].append(
                        ...
                        # hard macro pin
                        hard_macro_pin = self.HardMacroPin(
                            name=node_name,
                            ref_id=None,
                            x=attr_dict["x"][1],
                            y=attr_dict["y"][1],
                        ...
                        if hard_macro_name in self.hard_macros_to_inpins.keys():
                            self.hard_macros_to_inpins[hard_macro_name].append(
    ...
    def __read_plc(self, plc_pth: str):
        ...
        for cnt, line in enumerate(open(plc_pth, "r")):
            ...
            if "Columns" in line_item and "Rows" in line_item:
                ...
                it in line_item for it in ["Routes", "per", "micron", "hor", "ver"]
            ):
                ...
                it in line_item
                for it in ["Routes", "used", "by", "macros", "hor", "ver"]
            ):
                ...
                all(it in line_item for it in ["HARD", "MACROs"])
                and len(line_item) == 3
            ):
                _hard_macros_cnt = int(line_item[2])
            elif (
                all(it in line_item for it in ["HARD", "MACRO", "PINs"])
                and len(line_item) == 4
            ):
                _hard_macro_pins_cnt = int(line_item[3])
            elif all(it in line_item for it in ["PORTs"]) and len(line_item) == 2:
                _ports_cnt = int(line_item[1])
            elif (
                all(it in line_item for it in ["SOFT", "MACROs"])
                and len(line_item) == 3
            ):
                _soft_macros_cnt = int(line_item[2])
            elif (
                all(it in line_item for it in ["SOFT", "MACRO", "PINs"])
                and len(line_item) == 4
            ):
                _soft_macro_pins_cnt = int(line_item[3])
            elif all(it in line_item for it in ["STDCELLs"]) and len(line_item) == 2:
                _stdcells_cnt = int(line_item[1])
            elif all(it in line_item for it in ["MACROs"]) and len(line_item) == 2:
                _macros_cnt = int(line_item[1])
            elif (
                all(re.match(r"[0-9FNEWS\.\-]+", it) for it in line_item)
                and len(line_item) == 5
            ):
    ...
    def restore_placement(
        self, plc_pth: str, ifInital=True, ifValidate=False, ifReadComment=False
    ):
        """
        ...
        if ifValidate:
            try:
                ...
                print(
                    "[ERROR NETLIST/PLC MISMATCH] at line {} in statement {}".format(
                        line, text
                    )
                )
                exit(1)
        ...
        try:
            ...
            assert sorted(
                self.port_indices + self.hard_macro_indices + self.soft_macro_indices
            ) == sorted(list(info_dict["node_plc"].keys()))
        except AssertionError:
            print(
                ...
                len(list(info_dict["node_plc"].keys())),
            )
            exit(1)
        ...
        if ifReadComment:
            ...
            self.set_routes_per_micron(
                info_dict["routes_per_micron_hor"], info_dict["routes_per_micron_ver"]
            )
    ...
    def get_cost(self) -> float:
        ...
        return self.get_wirelength() / (
            (self.get_canvas_width_height()[0] + self.get_canvas_width_height()[1])
            * self.net_cnt
        )
    ...
    def __get_pin_position(self, pin_idx):
        ...
        try:
            assert (
                self.modules_w_pins[pin_idx].get_type() == "MACRO_PIN"
                or self.modules_w_pins[pin_idx].get_type() == "PORT"
            )
        except Exception:
            print(
                "[ERROR PIN POSITION] Not a MACRO PIN",
                self.modules_w_pins[pin_idx].get_name(),
            )
            exit(1)
        ...
        for driver_pin_name in self.nets.keys():
            ...
            if x_coord:
                total_hpwl += weight_fact * (
                    abs(max(x_coord) - min(x_coord)) + abs(max(y_coord) - min(y_coord))
                )
        return total_hpwl
    ...
    def get_V_congestion_cost(self) -> float:
        ...
        occupied_cells = sorted(
            [gc for gc in self.V_routing_cong if gc != 0.0], reverse=True
        )
        cong_cost = 0.0
    ...
    def get_H_congestion_cost(self) -> float:
        ...
        """
        occupied_cells = sorted(
            [gc for gc in self.H_routing_cong if gc != 0.0], reverse=True
        )
        cong_cost = 0.0
    ...
    def __place_node_mask(
        self, grid_cell_idx: int, mod_width: float, mod_height: float
    ):
        """
        ...
        hor_pad, ver_pad = self.__node_pad_cell(
            mod_width=mod_width, mod_height=mod_height
        )

        self.node_mask[
            row - ver_pad : row + ver_pad + 1, col - hor_pad : col + hor_pad + 1
        ] = 0
    ...
    def __add_module_to_grid_cells(self, mod_x, mod_y, mod_w, mod_h):
        ...
        for r_i in range(bl_row, ur_row + 1):
            for c_i in range(bl_col, ur_col + 1):
                ...
                self.grid_occupied[self.grid_col * r_i + c_i] += self.__overlap_area(
                    grid_cell_block, module_block
                )
    ...
    def get_grid_cells_density(self):
        ...
        for module_idx in self.soft_macro_indices + self.hard_macro_indices:
            ...
            self.__add_module_to_grid_cells(
                mod_x=module_x, mod_y=module_y, mod_h=module_h, mod_w=module_w
            )
    ...
    def get_density_cost(self) -> float:
        ...
        occupied_cells = sorted(
            [gc for gc in self.grid_cells if gc != 0.0], reverse=True
        )
        density_cost = 0.0
    ...
    def set_routes_per_micron(
        self, hroutes_per_micron: float, vroutes_per_micron: float
    ) -> None:
        """
        Set Routes per Micron
        """
        print(
            "#[ROUTES PER MICRON] Hor: %.2f, Ver: %.2f"
    ...
    def set_macro_routing_allocation(
        self, hrouting_alloc: float, vrouting_alloc: float
    ) -> None:
        """
    ...
    def __macro_route_over_grid_cell(self, mod_x, mod_y, mod_w, mod_h):
        ...
        for r_i in range(bl_row, ur_row + 1):
            for c_i in range(bl_col, ur_col + 1):
                ...
                self.V_macro_routing_cong[r_i * self.grid_col + c_i] += (
                    x_dist * self.vrouting_alloc
                )
                self.H_macro_routing_cong[r_i * self.grid_col + c_i] += (
                    y_dist * self.hrouting_alloc
                )
    ...
    def get_routing(self):
        ...
        for mod in self.modules_w_pins:
            ...
            if curr_type == "PORT" and mod.get_sink():
                    ...
                    for sink_pin in mod.get_sink()[sink_name]:
                        ...
                        node_gcells.add(
                            self.__get_grid_cell_location(
                                *(self.__get_pin_position(sink_idx))
                            )
                        )
                ...
                node_gcells.add(
                    self.__get_grid_cell_location(*(self.__get_pin_position(mod_idx)))
                )
                source_gcell = self.__get_grid_cell_location(
                    *(self.__get_pin_position(mod_idx))
                )
            ...
            elif curr_type == "MACRO" and self.is_node_hard_macro(
                self.mod_name_to_indices[mod.get_name()]
            ):
                module_h = mod.get_height()
                module_w = mod.get_width()
                module_x, module_y = mod.get_pos()
                # compute overlap
                self.__macro_route_over_grid_cell(
                    module_x, module_y, module_w, module_h
                )

            if len(node_gcells) == 2:
                self.__two_pin_net_routing(
                    source_gcell=source_gcell, node_gcells=node_gcells, weight=weight
                )
            elif len(node_gcells) == 3:
                self.__three_pin_net_routing(node_gcells=node_gcells, weight=weight)
            elif len(node_gcells) > 3:
                for curr_net in self.__split_net(
                    source_gcell=source_gcell, node_gcells=node_gcells
                ):
                    self.__two_pin_net_routing(
                        source_gcell=source_gcell, node_gcells=curr_net, weight=weight
                    )
        ...
        self.V_routing_cong = [
            sum(x) for x in zip(self.V_routing_cong, self.V_macro_routing_cong)
        ]
        self.H_routing_cong = [
            sum(x) for x in zip(self.H_routing_cong, self.H_macro_routing_cong)
        ]
    ...
    def is_node_soft_macro(self, node_idx) -> bool:
        ...
        try:
            ...
            print(
                "[ERROR INDEX OUT OF RANGE] Can not process index at {}".format(
                    node_idx
                )
            )
            exit(1)
    ...
    def get_node_mask(self, node_idx: int, node_name: str = None) -> list:
        ...
        canvas_block = Block(x_max=self.width, y_max=self.height, x_min=0, y_min=0)
        ...
        temp_node_mask = np.array([1] * (self.grid_col * self.grid_row)).reshape(
            self.grid_row, self.grid_col
        )
        ...
        for i in range(self.grid_row):
            for j in range(self.grid_col):
                ...
                if (
                    abs(
                        self.__overlap_area(block_i=canvas_block, block_j=mod_block)
                        - (mod_w * mod_h)
                    )
                    > 1e-8
                ):
                    ...
                    for pmod_idx in self.placed_macro:
                        ...
                        if (
                            self.__overlap_area(block_i=pmod_block, block_j=mod_block)
                            > 0
                        ):
                            temp_node_mask[i][j] = 0
    ...
    def get_node_type(self, node_idx: int) -> str:
        ...
        try:
            ...
            print(
                "[WARNING INDEX OUT OF RANGE] Can not process index at {}".format(
                    node_idx
                )
            )
            return None
    ...
    def get_node_width_height(self, node_idx: int):
        ...
        try:
            ...
            print(
                "[ERROR NODE FIXED] Found {}. Only 'MACRO', 'macro', 'STDCELL'".format(
                    mod.get_type()
                )
    ...
    def get_macro_adjacency(self) -> list:
        ...
        macro_adj = (
            [0]
            * (self.hard_macro_cnt + self.soft_macros_cnt)
            * (self.hard_macro_cnt + self.soft_macros_cnt)
        )
        assert len(macro_adj) == (self.hard_macro_cnt + self.soft_macros_cnt) * (
            self.hard_macro_cnt + self.soft_macros_cnt
        )
        ...
        for row_idx, module_idx in enumerate(sorted(module_indices)):
            ...
            for col_idx, h_module_idx in enumerate(sorted(module_indices)):
                ...
                macro_adj[
                    row_idx * (self.hard_macro_cnt + self.soft_macros_cnt) + col_idx
                ] = entry
                macro_adj[
                    col_idx * (self.hard_macro_cnt + self.soft_macros_cnt) + row_idx
                ] = entry
    ...
    def get_macro_and_clustered_port_adjacency(self):
        ...
        macro_adj = (
            [0]
            * (len(module_indices) + len(clustered_ports))
            * (len(module_indices) + len(clustered_ports))
        )
        ...
        for row_idx, module_idx in enumerate(sorted(module_indices)):
            ...
            for col_idx, h_module_idx in enumerate(sorted(module_indices)):
                ...
                macro_adj[
                    row_idx * (len(module_indices) + len(clustered_ports)) + col_idx
                ] = entry
                macro_adj[
                    col_idx * (len(module_indices) + len(clustered_ports)) + row_idx
                ] = entry

        # instantiate clustered ports
        for row_idx, cluster_cell in enumerate(
            sorted(clustered_ports, key=lambda tup: tup[1])
        ):
            # add cell location
    ...
    def is_node_fixed(self, node_idx: int):
        ...
        try:
            ...
            print(
                "[ERROR NODE FIXED] Found {}. Only 'MACRO', 'STDCELL'".format(
                    mod.get_type()
                )
                + "'PORT' are considered to be fixable nodes"
            )
    ...
    def update_node_coords(self, node_idx, x_pos, y_pos):
        ...
        try:
            ...
            print(
                "[ERROR NODE LOCATION] Found {}. Only 'MACRO', 'macro', 'STDCELL'".format(
                    mod.get_type()
                )
    ...
    def update_macro_orientation(self, node_idx, orientation):
        ...
        try:
            ...
            print(
                "[ERROR MACRO ORIENTATION] Found {}. Only 'MACRO'".format(
                    mod.get_type()
                )
    ...
    def get_node_location(self, node_idx):
        ...
        try:
            ...
            print(
                "[ERROR NODE LOCATION] Found {}. Only 'MACRO', 'STDCELL'".format(
                    mod.get_type()
                )
                + "'PORT' are considered to be placable nodes"
            )
    ...
    def get_grid_cell_of_node(self, node_idx):
        ...
        try:
            ...
            print(
                "[ERROR NODE LOCATION] Found {}. Only 'MACRO'".format(mod.get_type())
                + " can be called"
            )
            exit(1)
        except Exception:
    ...
    def unfix_node_coord(self, node_idx):
        ...
        try:
            ...
            print(
                "[ERROR UNFIX NODE] Found {}. Only 'MACRO', 'STDCELL'".format(
                    mod.get_type()
                )
                + "'PORT' are considered to be fixable nodes"
            )
    ...
    def fix_node_coord(self, node_idx):
        ...
        try:
            ...
            print(
                "[ERROR FIX NODE] Found {}. Only 'MACRO', 'STDCELL'".format(
                    mod.get_type()
                )
                + "'PORT' are considered to be fixable nodes"
            )
    ...
    def __node_pad_cell(self, mod_width, mod_height):
        ...
        cell_hor = math.ceil(
            ((mod_width / 2) - (self.grid_width / 2)) / self.grid_width
        )
        cell_ver = math.ceil(
            ((mod_height / 2) - (self.grid_height / 2)) / self.grid_height
        )
    ...
    def place_node(self, node_idx, grid_cell_idx):
        ...
        try:
            ...
            print(
                "[ERROR PLACE NODE] Found {}. Only 'MACRO', 'STDCELL'".format(
                    mod.get_type()
                )
                + "'PORT' are considered to be placable nodes"
            )
        ...
        if not mod.get_fix_flag():
            ...
            self.__place_node_mask(
                grid_cell_idx, mod_width=mod.get_width(), mod_height=mod.get_height()
            )
    ...
    def unplace_node(self, node_idx):
        ...
        try:
            ...
            print(
                "[ERROR UNPLACE NODE] Found {}. Only 'MACRO', 'STDCELL'".format(
                    mod.get_type()
                )
                + "'PORT' are considered to be placable nodes"
            )
    ...
    def is_node_placed(self, node_idx):
        ...
        try:
            ...
            print(
                "[ERROR NODE PLACED] Found {}. Only 'MACRO', 'STDCELL',".format(
                    mod.get_type()
                )
                + "'PORT' are considered to be placable nodes"
            )
    ...
    def get_ref_node_id(self, node_idx=-1):
        ...
        if node_idx != -1:
            if (
                node_idx in self.soft_macro_pin_indices
                or node_idx in self.hard_macro_pin_indices
            ):
    ...
    def save_placement(self, filename, info=""):
        ...
        with open(filename, "w+") as f:
            ...
            for mod_idx in sorted(
                self.hard_macro_indices + self.soft_macro_indices + self.port_indices
            ):
                # [node_index] [x] [y] [orientation] [fixed]
                mod = self.modules_w_pins[mod_idx]
    ...
    def display_canvas(self, annotate=True, amplify=False, saveName=None, show=True):
        """
        ...
        for mod in self.modules_w_pins:
            if mod.get_type() == "PORT" and mod.get_placed_flag():
                ...
                if not self.is_node_soft_macro(
                    self.mod_name_to_indices[mod.get_name()]
                ):
                    # hard macro
                    ax.add_patch(
                        Rectangle(
                            (
                                mod.get_pos()[0] - mod.get_width() / 2,
                                mod.get_pos()[1] - mod.get_height() / 2,
                            ),
                            mod.get_width(),
                            mod.get_height(),
                            alpha=0.5,
                ...
                if macro.get_placed_flag():
                    plt.plot(
                        *self.__get_pin_position(pin_idx), "bo", markersize=PIN_SIZE
                    )
    ...
    def __repulsive_force(
        self, repel_factor, mod_i_idx, mod_j_idx, with_initialization=False
    ):
        """
    ...
    def __repulsive_force_hard_macro(self, repel_factor, h_node_i, s_node_j):
        ...
        if hypo_dist <= 1e-10 or self.__ifOverlap(h_node_i, s_node_j):
            return x_dist / hypo_dist * (
                h_mod_i.get_height() / 2 + s_mod_j.get_height() / 2
            ), y_dist / hypo_dist * (
                h_mod_i.get_height() / 2 + s_mod_j.get_height() / 2
            )
        else:
    ...
    def __fd_placement(
        ...
        def update_location(mod_id, x_disp, y_disp):
            ...
            with open("os_debug.txt", "a+") as the_file:
                the_file.write(
                    "{} {} {} {} {}\n".format(
                        mod_id, x_pos + x_disp, y_pos + y_disp, x_disp, y_disp
                    )
                )
        ...
        def move_soft_macros(attract_factor, repel_factor, io_factor, max_displacement):
            ...
            if max_x_disp > 0.0:
                for mod_idx in self.soft_macro_indices:
                    soft_macro_disp[mod_idx][0] = (
                        soft_macro_disp[mod_idx][0] / max_x_disp
                    ) * max_displacement
            if max_y_disp > 0.0:
                for mod_idx in self.soft_macro_indices:
                    soft_macro_disp[mod_idx][1] = (
                        soft_macro_disp[mod_idx][1] / max_y_disp
                    ) * max_displacement
            for mod_idx in self.soft_macro_indices:
                update_location(
                    mod_idx, soft_macro_disp[mod_idx][0], soft_macro_disp[mod_idx][1]
                )
        ...
        def calcAttractiveForce(attract_factor, io_factor, max_displacement):
            for driver_pin_name in self.nets.keys():
                ...
                for sink_pin_name in self.nets[driver_pin_name]:
                    ...
                    if (
                        sink_pin_idx in self.port_indices
                        or driver_pin_idx in self.port_indices
                    ):
        ...
        for i in range(len(num_steps)):
            ...
            for j in range(num_step):
                ...
                move_soft_macros(
                    attractive_factor, repulsive_factor, io_factor, max_displacement
                )

    def optimize_stdcells(
        self,
        use_current_loc,
        move_stdcells,
        move_macros,
    ...
    class SoftMacroPin:
        def __init__(self, name, ref_id, x=0.0, y=0.0, macro_name="", weight=1.0):
            self.name = name
            self.ref_id = ref_id
            self.x = float(x)
            self.y = float(y)
...
def main():
    test_netlist_dir = "./Plc_client/test/" + "ariane_68_1.3"
    ...
    print(
        "# MACRO_PINs      :         %d"
        % (plc.get_hard_macro_pins_count() + plc.get_soft_macro_pins_count())
    )
